...
#
# This function simulates data from a sinusoidal function, adds random noise, and performs local polynomial regression on the simulated data
sim.sinus<- function(n=100, nt=100, sigma.e =.2, S=1, h=.2,
                     q=1,doing.plot=TRUE, doing.plot.locpolreg=FALSE, ...){
    x <- seq(0,1,length=n) # create a sequence of n equally spaced values
    mx <- sin(2*pi*x) # create sinusoidal function mx based on x values
    
    tg <- seq(0,1,length=nt) # create timegrid for plotting with nt points
    mtg <- sin(2*pi*tg) # creates true sinusoidal function based on time grid

    mean.hmtg <- 0*mtg
    mean.hmtg2 <- 0*mtg
  
    # local polynomial regression loop
    for (i in seq(1:S)){
       ...
       mean.hmtg <- mean.hmtg + hmtg.aux # update mean of the regression function
       mean.hmtg2 <- mean.hmtg2 + hmtg.aux*hmtg.aux # updatd squared mean
    }
    mean.hmtg <- mean.hmtg/S
    mean.hmtg2 <- mean.hmtg2/S
    sd.hmtg <- sqrt(mean.hmtg2-mean.hmtg*mean.hmtg)
    if (doing.plot){
      plot(tg,mtg,type="l",ylim=c(-1.2,1.2)) # plot true sinusoidal function
      lines(tg,mean.hmtg,col=4)
      lines(tg,mean.hmtg+2*sd.hmtg,col=3,lty=2) # confidence inteval
      lines(tg,mean.hmtg-2*sd.hmtg,col=3,lty=2)  # confidence inteval
      lines(tg,mtg,col=1)
    }
...
for (h in h.v){
    ...
    # create timegrid and tru sinusoidal function
    ...
    # run S simulariont of the local polynomial regression
    for (i in seq(1:S)){
       y <- mx + rnorm(n,0,sigma.e)
       hmtg.aux <- locpolreg(x=x,y=y,h=h,q=q,r=0,tg=tg,ylim=c(-1.2,1.2),
...
# iterate over bandwidths and simulate estimation with sim.sinus
for (h in h.v){
...
sim.sinus.hv <- function(n=100, nt=100, sigma.e =.2, S=1, hv=c(.2,.4), q=1, ...){
  ...
  tg <- seq(0,1,length=nt) 
  mtg <- sin(2*pi*tg) # treu sinusoidal function
  
  nh <- length(hv) # number of bandwidth values
  ...
  for (i in seq(1:S)){
    y <- mx + rnorm(n,0,sigma.e) # generate simulated y data
    for (j in (1:nh)){
      # perform local polynomial regression for each bandwidth in each simulation S
      h <- hv[j]
  ...
  mean.hmtg  <- mean.hmtg/S # calculate mean 
  mean.hmtg2 <- mean.hmtg2/S
  sd.hmtg <- sqrt(mean.hmtg2-mean.hmtg*mean.hmtg) # calculate standard deviation
  dt <- (tg[2]-tg[1])
  IntBias2 <- apply(mean.hmtg, 1, function(x,mtg,dt){sum((x-mtg)^2)*dt},mtg,dt) # calculate integrated bias squared
  IntVar <- apply(sd.hmtg,1,function(x,dt){sum(x^2)*dt},dt) # calculate integrated variance
  IMSE <- IntBias2 + IntVar # calculate integrated mean squared error
