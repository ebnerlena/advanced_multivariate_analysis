...
```{r}
# define formular for gam function
# lg.IMR is response var
# lg.IMR ~ lg.LE -> would be fitting a linear model = lm(lg.IMR ~ lg.LE)

# lm_gam_s_tp <- gam(lg.IMR ~ lg.LE)
# lm_s_tp <- lm(lg.IMR ~ lg.LE)
# summary(lm_gam_s_tp)
# summary(lm_s_tp)

# using gam we fit a nonparametric model, spline regression with only 1 explanatory variable
# function as has several parameters ?s e.g. if we use fx=True we use a regression spline without penalty
...
plot(spl_gam_s_tp, residuals = TRUE,
     ...
     shift = spl_gam_s_tp$coefficients[1], # if we donot define this we do get a function without the intercept
     seWithMean=TRUE,
...
```{r}
# when we don't use k, it takes the default k which is 10 for 1 explanatory variable
spl_gam_s_cr <- gam(lg.IMR ~ s(lg.LE,bs="cr")) # using a cubic regression spline (default is ts thin plate regression spline)
...
```{r}
spl_gam_te_cr <- gam(lg.IMR ~ te(lg.LE,bs="cr",k=15)) # by default, k=10
