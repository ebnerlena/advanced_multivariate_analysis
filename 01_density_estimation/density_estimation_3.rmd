...
### 3.1 Bivariate density estimation. 

```{r}
library(sm)
op <- par(mfrow=c(1,2))
sm.density(cbind(Boston$lstat,Boston$rm),h=c(1.5,.15),
           phi=30,theta=60,col=5,
...
### 3.2 Density estimation for a 3-dim dataset. 

Library `ks` includes, amonf other, the function `kde` for density estimations in dimensions 1 to 6, as well as a method of `plot` for representing graphically the estimated dnsities. 
See `help(kde)` and `help(plot.kde)` and run the included examples. 

Consider again the Boston dataset. 
The following code estimates the joint density of 3 variables: `(age, lstat, medv)`:
```{r}
# names(Boston[,c(7,13,14)])
# [1] "age"   "lstat" "medv" 
require("ks")
d3Boston <- kde(Boston[,c(7,13,14)])
plot(d3Boston,phi=20,theta=210)
```

```{r,eval=FALSE}
require(rgl)
plot(d3Boston,display="rgl")
```
