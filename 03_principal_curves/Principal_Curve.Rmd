...
points(x,col=4)
#lines(fit)
points(fit) # points of principal curve
...
# lambda = for each point, its arclemgth from the beginning of the curve = score, lambdas are not sorted accoridng to their place on curve
# dist = total sum of squared distances from the points to their projections on curve
# converged = a boolean wether algorithm converged or not
...
```{r }
# We analyze digits "0"
zip.train <- read.table("03_principal_curves/zip.train")
I.0 <- (zip.train[,1]==0)
zip.0 <- zip.train[I.0,]
n<-dim(zip.0)[1]
