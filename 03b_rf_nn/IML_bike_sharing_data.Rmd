...
author: "Pedro Delicado, Universitat Politècnica de Catalunya - Barcelona TECH"
date: "`r Sys.Date()`"
...
split_criterion <- function(y, split.var=y){
  y <- y[order(split.var)] # sort values based on split.var
  n <- length(y)
  uni.spl.var <- sort(unique(split.var)) # only consider unqiue values
  m <- length(uni.spl.var)
  Q <- numeric(m+1)
  Q[1] <- Q[m+1] <- (n-1)*var(y) # within-group variance using a weighted sum of variance
  #Q[2] <- (n-2)*var(y[2:n])
  for (i in 2:m){
  ...
  #Q[m] <- (n-2)*var(y[1:(n-1)])
  Q <- Q/Q[1] # normalize by total variance Q[1]
  wminQ <- which.min(Q)
...
Rel_Gh_Var <- relev.ghost.var(model=gam_bikes, 
                              newdata = day_sel_test[, -1],
                              y.ts = day_sel_test[, 1], # data used to evaluate model performance
