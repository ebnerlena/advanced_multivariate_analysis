...
pred<-seq(0,40,length.out=1000) # generating a sequence of 1000 evenly spaced values ranging from 0 to 40
...
# )
  ...
  #folds <- cut(seq(1, nrow(bikes)), breaks = n, labels = FALSE)
  # Set a random seed for reproducibility
  set.seed(123)
  # Create a vector of indices from 1 to the number of rows in the dataset
  indices <- seq(1, nrow(data))
  # Randomly shuffle the indices
  shuffled_indices <- sample(bikes[,1])
  # Divide the shuffled indices into k folds
...
for (j in  dff){
  for ( i in seq(k)){
    # Create training and validation sets
      #validation_indices <- which(df$folds == i, arr.ind = TRUE)
