...
```{r}
table(km.3$cluster,kmed.3.Eucl$cluster) # compare clusters by cross-table
# counts how many observations fall into each combination of clusters
# provides insights into agreement or disagreement between the 2 clustering methods
# diagonal elements represent cases where the methods agree on the cluster, of-diagonal elements represent cases where the methods disagree

# first row second column (1,2) has count of 58 means that 58 observation were assigned to cluster 2 by pam and cluster 17 by the k-means methods
# third row first column has count 33, 33 observations were assigned to cluster 1 by PAM and 38 to cluster 3 by k-means
```
...
That is,
\[
K^\*=\arg\min*{K}\{K: G(K)\ge G(K+1)-\mbox{sd}*{K+1}\}.
\]
...
# look for a point where the AIC start to decrease more slowly -> indicates a good trade-off between model complexity and fit
```
