...
X <- scale(Boston[,c(13,6)]) # scale data with standard deviation
...
1. We want to estimate the joint bivariate density using a kernel estimator with the same bandwidth in both dimensions: $h=(a,a)$. 
...
# kernel density estimator result with optimal bandwidth of 0.3
```
...
# asp is used for aspect ratio between axes
```
```{r,fig.asp=1}
