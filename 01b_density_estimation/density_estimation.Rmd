...
hcdr <- hist(CDrate,breaks=seq(7.3999,8.7999,by=.2),fre=F) # freq=F = using density instead of frequency
...
hist(Boston$lstat,br=breaks,freq=F,xlim=c(0,40),ylim=c(0,.08)) # use freq=False is very important if using different bin widths
par(op)
```
...
sm.density(cbind(Boston$lstat,Boston$rm),h=c(1.5,.15),display="slice",
           xlab="lstat",ylab="rm")
sm.density(cbind(Boston$lstat,Boston$rm),h=c(1.5,.15),display="slice",add=T,col=2,props=c(90)) # props = specify the probability level s for which the contours will be drawn e.g. 90%
